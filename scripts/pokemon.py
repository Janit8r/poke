...
def safe_request(url, headers, max_retries=3, delay=2):
  """安全的网络请求，带有重试机制"""
...
def get_single_evolution_chain(tr_list):
  ...
  def get_pokemon(td):
    ...
    name = name_el.text
    form_name = None
    image = image_el.get('href').split('File:')[1] if image_el else None
...
def get_home_images(soup, name, index):
  ...
  for td in td_list:
    ...
    for el in extra_image_el_list:
      alt_text = el.get('alt')
      if alt_text and '糖饰' in alt_text:
        extra_name = f'''-{alt_text}'''
...
if __name__ == '__main__':
  ...
  if data:
    ...
    print(f"成功保存宝可梦数据: {name}")
  else:
    print(f"无法获取宝可梦数据，跳过保存: {name}")
